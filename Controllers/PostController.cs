...
public class PostController : ControllerBase
{
        ...
        [FromQuery] int pageSize = 25)
    {
        try
        {
            var count = await context.Post.AsNoTracking().CountAsync();
            var posts = await context.Post
                .AsNoTracking()
                .Include(x => x.Category)
                .Include(x => x.Author)
                .Select(x => new ListViewModel //para utilizar esse select explicito comentar em Program, .AddJsonOptions
                {
                    Id = x.Id,
                    Title = x.Title,
                    Slug = x.Slug,
